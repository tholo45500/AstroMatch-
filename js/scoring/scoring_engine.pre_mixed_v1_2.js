...
export async function loadWeightingConfig() {
  const mod = await import("./config/weighting_v1.json", { with: { type: "json" } });
...
export function computeScore(synastry, weightingConfig) {
  ...
  const domainScores = domains.map((domain) => {
    ...
    const rawPositive = contributions.filter((c) => c.final_points >= 0).reduce((acc, c) => acc + c.final_points, 0);
    const rawNegative = contributions.filter((c) => c.final_points < 0).reduce((acc, c) => acc + Math.abs(c.final_points), 0);
  ...
  return {
    ...
    computed_at: nowIso(),
    partial: synastry.reliability.degraded_mode,
